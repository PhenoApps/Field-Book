...
public final class USBMonitor {
	...
	/**
	 * ã‚³ãƒ¼ãƒ«ãƒãƒƒã‚¯ã‚’ãƒ¯ãƒ¼ã‚«ãƒ¼ã‚¹ãƒ¬ãƒƒãƒ‰ã§å‘¼ã³å‡ºã™ãŸã‚ã®ãƒãƒ³ãƒ‰ãƒ©ãƒ¼
	 */
	private final Handler mAsyncHandler;
	private volatile boolean destroyed;
	/**
	 * USBæ©Ÿå™¨ã®çŠ¶æ…‹å¤‰æ›´æ™‚ã®ã‚³ãƒ¼ãƒ«ãƒãƒƒã‚¯ãƒªã‚¹ãƒŠãƒ¼
	 */
	...
	public void destroy() {
		...
		if (!destroyed) {
			destroyed = true;
			// ãƒ¢ãƒ‹ã‚¿ãƒ¼ã—ã¦ã„ã‚‹USBæ©Ÿå™¨ã‚’å…¨ã¦closeã™ã‚‹
	...
	public synchronized void unregister() throws IllegalStateException {
		// æŽ¥ç¶šãƒã‚§ãƒƒã‚¯ç”¨Runnableã‚’å‰Šé™¤
		mDeviceCounts = 0;
		if (!destroyed) {
...
//			if (DEBUG) Log.i(TAG, "unregister:");
	 ...
	 * @param device
	 * @return true: æŒ‡å®šã—ãŸUsbDeviceã«ãƒ‘ãƒ¼ãƒŸãƒƒã‚·ãƒ§ãƒ³ãŒã‚ã‚‹
	 * @throws IllegalStateException
	 */
	...
	/**
	 * å†…éƒ¨ã§ä¿æŒã—ã¦ã„ã‚‹ãƒ‘ãƒ¼ãƒŸãƒƒã‚·ãƒ§ãƒ³çŠ¶æ…‹ã‚’æ›´æ–°
	 * @param device
	 * @param hasPermission
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
		...
		if (isRegistered()) {
			if (device != null) {
				if (mUsbManager.hasPermission(device)) {
					...
					try {
						// ãƒ‘ãƒ¼ãƒŸãƒƒã‚·ãƒ§ãƒ³ãŒãªã‘ã‚Œã°è¦æ±‚ã™ã‚‹
						mUsbManager.requestPermission(device, mPermissionIntent);
					} catch (final Exception e) {
						// Android5.1.xã®GALAXYç³»ã§android.permission.sec.MDM_APP_MGMTã¨ã„ã†æ„å‘³ä¸æ˜Žã®ä¾‹å¤–ç”Ÿæˆã™ã‚‹ã¿ãŸã„
						Log.w(TAG, e);
	...
	/**
	 * æŒ‡å®šã—ãŸUsbDeviceã‚’openã™ã‚‹
	 * @param device
	 * @return
	 * @throws SecurityException ãƒ‘ãƒ¼ãƒŸãƒƒã‚·ãƒ§ãƒ³ãŒãªã‘ã‚Œã°SecurityExceptionã‚’æŠ•ã’ã‚‹
	 */
	public UsbControlBlock openDevice(final UsbDevice device) throws SecurityException {
		if (hasPermission(device)) {
			...
			if (result == null) {
				result = new UsbControlBlock(USBMonitor.this, device);    // ã“ã®ä¸­ã§openDeviceã™ã‚‹
		...
		public void onReceive(final Context context, final Intent intent) {
			...
			if (ACTION_USB_PERMISSION.equals(action)) {
				...
				if (device != null) {
					...
					if (ctrlBlock != null) {
						// cleanup
						ctrlBlock.close(true, false); // hot-unplug: don't close dead FD
					}
					mDeviceCounts = 0;
	...
	/**
	 * USBæ©Ÿå™¨æ¯Žã®è¨­å®šä¿å­˜ç”¨ã«ãƒ‡ãƒã‚¤ã‚¹ã‚­ãƒ¼åã‚’ç”Ÿæˆã™ã‚‹ã€‚
	 * ãƒ™ãƒ³ãƒ€ãƒ¼ID, ãƒ—ãƒ­ãƒ€ã‚¯ãƒˆID, ãƒ‡ãƒã‚¤ã‚¹ã‚¯ãƒ©ã‚¹, ãƒ‡ãƒã‚¤ã‚¹ã‚µãƒ–ã‚¯ãƒ©ã‚¹, ãƒ‡ãƒã‚¤ã‚¹ãƒ—ãƒ­ãƒˆã‚³ãƒ«ã‹ã‚‰ç”Ÿæˆ
	 * åŒç¨®ã®è£½å“ã ã¨åŒã˜ã‚­ãƒ¼åã«ãªã‚‹ã®ã§æ³¨æ„
	 * @param device nullãªã‚‰ç©ºæ–‡å­—åˆ—ã‚’è¿”ã™
	 * @return
	 */
	...
	/**
	 * USBæ©Ÿå™¨æ¯Žã®è¨­å®šä¿å­˜ç”¨ã«ãƒ‡ãƒã‚¤ã‚¹ã‚­ãƒ¼åã‚’ç”Ÿæˆã™ã‚‹ã€‚
	 * useNewAPI=falseã§åŒç¨®ã®è£½å“ã ã¨åŒã˜ãƒ‡ãƒã‚¤ã‚¹ã‚­ãƒ¼ã«ãªã‚‹ã®ã§æ³¨æ„
	 * @param device
	 * @param useNewAPI
	 * @return
	 */
	...
	/**
	 * USBæ©Ÿå™¨æ¯Žã®è¨­å®šä¿å­˜ç”¨ã«ãƒ‡ãƒã‚¤ã‚¹ã‚­ãƒ¼åã‚’ç”Ÿæˆã™ã‚‹ã€‚ã“ã®æ©Ÿå™¨åã‚’HashMapã®ã‚­ãƒ¼ã«ã™ã‚‹
	 * UsbDeviceãŒopenã—ã¦ã„ã‚‹æ™‚ã®ã¿æœ‰åŠ¹
	 * ãƒ™ãƒ³ãƒ€ãƒ¼ID, ãƒ—ãƒ­ãƒ€ã‚¯ãƒˆID, ãƒ‡ãƒã‚¤ã‚¹ã‚¯ãƒ©ã‚¹, ãƒ‡ãƒã‚¤ã‚¹ã‚µãƒ–ã‚¯ãƒ©ã‚¹, ãƒ‡ãƒã‚¤ã‚¹ãƒ—ãƒ­ãƒˆã‚³ãƒ«ã‹ã‚‰ç”Ÿæˆ
	 * serialãŒnullã‚„ç©ºæ–‡å­—ã§ãªã‘ã‚Œã°serialã‚’å«ã‚ãŸãƒ‡ãƒã‚¤ã‚¹ã‚­ãƒ¼åã‚’ç”Ÿæˆã™ã‚‹
	 * useNewAPI=trueã§APIãƒ¬ãƒ™ãƒ«ã‚’æº€ãŸã—ã¦ã„ã‚Œã°ãƒžãƒ‹ãƒ¥ãƒ•ã‚¡ã‚¯ãƒãƒ£å, ãƒãƒ¼ã‚¸ãƒ§ãƒ³, ã‚³ãƒ³ãƒ•ã‚£ã‚®ãƒ¥ãƒ¬ãƒ¼ã‚·ãƒ§ãƒ³ã‚«ã‚¦ãƒ³ãƒˆã‚‚ä½¿ã†
	 * @param device nullãªã‚‰ç©ºæ–‡å­—åˆ—ã‚’è¿”ã™
	 * @param serial	UsbDeviceConnection#getSerialã§å–å¾—ã—ãŸã‚·ãƒªã‚¢ãƒ«ç•ªå·ã‚’æ¸¡ã™, nullã§useNewAPI=trueã§API>=21ãªã‚‰å†…éƒ¨ã§å–å¾—
	 * @param useNewAPI API>=21ã¾ãŸã¯API>=23ã®ã¿ã§ä½¿ç”¨å¯èƒ½ãªãƒ¡ã‚½ãƒƒãƒ‰ã‚‚ä½¿ç”¨ã™ã‚‹(ãŸã ã—æ©Ÿå™¨ã«ã‚ˆã£ã¦ã¯nullãŒè¿”ã£ã¦ãã‚‹ã®ã§æœ‰åŠ¹ã‹ã©ã†ã‹ã¯æ©Ÿå™¨ã«ã‚ˆã‚‹)
	 * @return
	 */
	@SuppressLint("NewApi")
...
//		if (DEBUG) Log.v(TAG, "getDeviceKeyName:" + sb.toString());
	...
	/**
	 * ãƒ‡ãƒã‚¤ã‚¹ã‚­ãƒ¼ã‚’æ•´æ•°ã¨ã—ã¦å–å¾—
	 * getDeviceKeyNameã§å¾—ã‚‰ã‚Œã‚‹æ–‡å­—åˆ—ã®hasCodeã‚’å–å¾—
	 * ãƒ™ãƒ³ãƒ€ãƒ¼ID, ãƒ—ãƒ­ãƒ€ã‚¯ãƒˆID, ãƒ‡ãƒã‚¤ã‚¹ã‚¯ãƒ©ã‚¹, ãƒ‡ãƒã‚¤ã‚¹ã‚µãƒ–ã‚¯ãƒ©ã‚¹, ãƒ‡ãƒã‚¤ã‚¹ãƒ—ãƒ­ãƒˆã‚³ãƒ«ã‹ã‚‰ç”Ÿæˆ
	 * åŒç¨®ã®è£½å“ã ã¨åŒã˜ãƒ‡ãƒã‚¤ã‚¹ã‚­ãƒ¼ã«ãªã‚‹ã®ã§æ³¨æ„
	 * @param device nullãªã‚‰0ã‚’è¿”ã™
	 * @return
	 */
	 ...
	 * ãƒ‡ãƒã‚¤ã‚¹ã‚­ãƒ¼ã‚’æ•´æ•°ã¨ã—ã¦å–å¾—
	 * getDeviceKeyNameã§å¾—ã‚‰ã‚Œã‚‹æ–‡å­—åˆ—ã®hasCodeã‚’å–å¾—
	 * serialãŒnullã§useNewAPI=falseã§åŒç¨®ã®è£½å“ã ã¨åŒã˜ãƒ‡ãƒã‚¤ã‚¹ã‚­ãƒ¼ã«ãªã‚‹ã®ã§æ³¨æ„
	 * @param device nullãªã‚‰0ã‚’è¿”ã™
	 * @param serial UsbDeviceConnection#getSerialã§å–å¾—ã—ãŸã‚·ãƒªã‚¢ãƒ«ç•ªå·ã‚’æ¸¡ã™, nullã§useNewAPI=trueã§API>=21ãªã‚‰å†…éƒ¨ã§å–å¾—
	 * @param useNewAPI API>=21ã¾ãŸã¯API>=23ã®ã¿ã§ä½¿ç”¨å¯èƒ½ãªãƒ¡ã‚½ãƒƒãƒ‰ã‚‚ä½¿ç”¨ã™ã‚‹(ãŸã ã—æ©Ÿå™¨ã«ã‚ˆã£ã¦ã¯nullãŒè¿”ã£ã¦ãã‚‹ã®ã§æœ‰åŠ¹ã‹ã©ã†ã‹ã¯æ©Ÿå™¨ã«ã‚ˆã‚‹)
	 * @return
	...
	/**
	 * æŒ‡å®šã—ãŸIDã®Stringãƒ‡ã‚£ã‚¹ã‚¯ãƒªãƒ—ã‚¿ã‹ã‚‰æ–‡å­—åˆ—ã‚’å–å¾—ã™ã‚‹ã€‚å–å¾—ã§ããªã‘ã‚Œã°null
	 * @param connection
	 * @param id
	 * @param languageCount
	 * @param languages
		...
		for (int i = 1; i <= languageCount; i++) {
			...
			if ((ret > 2) && (work[0] == ret) && (work[1] == USB_DT_STRING)) {
				...
				try {
					...
					if (!"Ð‰".equals(result)) {	// å¤‰ãªã‚´ãƒŸãŒè¿”ã£ã¦ãã‚‹æ™‚ãŒã‚ã‚‹
						break;
					} else {
						result = null;
					}
	...
	/**
	 * ãƒ™ãƒ³ãƒ€ãƒ¼åãƒ»è£½å“åãƒ»ãƒãƒ¼ã‚¸ãƒ§ãƒ³ãƒ»ã‚·ãƒªã‚¢ãƒ«ã‚’å–å¾—ã™ã‚‹
	 * @param device
	 * @return
	 */
	...
	/**
	 * ãƒ™ãƒ³ãƒ€ãƒ¼åãƒ»è£½å“åãƒ»ãƒãƒ¼ã‚¸ãƒ§ãƒ³ãƒ»ã‚·ãƒªã‚¢ãƒ«ã‚’å–å¾—ã™ã‚‹
	 * #updateDeviceInfo(final UsbManager, final UsbDevice, final UsbDeviceInfo)ã®ãƒ˜ãƒ«ãƒ‘ãƒ¼ãƒ¡ã‚½ãƒƒãƒ‰
	 * @param context
	 * @param device
	 * @return
	 */
...
//			}
		...
		private UsbControlBlock(final UsbControlBlock src) throws IllegalStateException {
			...
			mBusNum = src.mBusNum;
			mDevNum = src.mDevNum;
			// FIXME USBMonitor.mCtrlBlocksã«è¿½åŠ ã™ã‚‹(ä»Šã¯HashMapãªã®ã§è¿½åŠ ã™ã‚‹ã¨ç½®ãæ›ã‚ã£ã¦ã—ã¾ã†ã®ã§ã ã‚, Listã‹HashMapã«Listã‚’ã¶ã‚‰ä¸‹ã’ã‚‹?)
		}
		...
		/**
		 * @param notify when false, close without firing onDisconnect
		 *               (used by UVCCamera when releasing a cloned control block).
		 */
		public synchronized void close(final boolean notify) {
			close(notify, true);
		}

		/**
		 * @param notify when false, skip onDisconnect
		 * @param closeConnection when false, skip UsbDeviceConnection.close()
		 *                        (hot-unplug: FD already dead; closing races libusb).
		 */
		public synchronized void close(final boolean notify, final boolean closeConnection) {
			if (DEBUG) Log.i(TAG, "UsbControlBlock#close:notify=" + notify + " closeConnection=" + closeConnection);

			if (mConnection != null) {
				final USBMonitor monitor = mWeakMonitor.get();
				if (notify && monitor != null) {
					// Notify before closing the FD so clients can stop the native pump.
				...
				// Connection already cleared; still notify once if needed.
				final USBMonitor monitor = mWeakMonitor.get();
				if (monitor != null && monitor.mOnDeviceConnectListener != null) {
					monitor.mOnDeviceConnectListener.onDisconnect(mWeakDevice.get(), UsbControlBlock.this);
				}
			}
		}
