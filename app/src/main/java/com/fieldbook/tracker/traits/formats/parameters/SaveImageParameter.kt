...
class SaveImageParameter(private val initialDefaultValue: Boolean? = null) :
    BaseFormatParameter(
    ...
    inner class ViewHolder(itemView: View) : BaseFormatParameter.ViewHolder(itemView) {
        ...
        override fun merge(traitObject: TraitObject) = traitObject.apply {
            saveImage = defaultValueToggle.isChecked
        }

        override fun load(traitObject: TraitObject?): Boolean {
            try {
                defaultValueToggle.isChecked = traitObject?.saveImage == true
            } catch (e: Exception) {
                e.printStackTrace()
                return false
            }
