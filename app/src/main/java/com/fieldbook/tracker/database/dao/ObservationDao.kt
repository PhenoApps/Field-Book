...
//        } ?: emptyArray()
        ...
        // /**
        // * Get all observations for a field and categorize them by type and status
        // * @param fieldId The field ID to get observations for
        // * @return A map containing categorized observations and counts
        // */
        // @SuppressLint("Recycle")
        // fun getBrAPIExportData(fieldId: Int, hostUrl: String): Map<String, Any> = withDatabase { db ->
        //     // Create result containers
        //     val categories = mapOf(
        //         "newObservations" to mutableListOf<Observation>(),
        //         "syncedObservations" to mutableListOf<Observation>(),
        //         "editedObservations" to mutableListOf<Observation>(),
        //         "newImageObservations" to mutableListOf<Observation>(),
        //         "syncedImageObservations" to mutableListOf<Observation>(),
        //         "editedImageObservations" to mutableListOf<Observation>(),
        //         "incompleteImageObservations" to mutableListOf<Observation>(),
        //         "userCreatedTraitObservations" to mutableListOf<Observation>(),
        //         "wrongSourceObservations" to mutableListOf<Observation>(),
        //         "userCreatedImageObservations" to mutableListOf<Observation>(),
        //         "wrongSourceImageObservations" to mutableListOf<Observation>()
        //     )
            
        //     // Query to get all observations for the field
        //     val sql = """
        //         SELECT 
        //             obs.${Observation.PK} AS id,
        //             obs.value,
        ...
        //     db.rawQuery(sql, arrayOf(fieldId.toString())).use { cursor ->
        //         while (cursor.moveToNext()) {
        //             val observation = Observation().apply {
        //                 fieldBookDbId = cursor.getString(cursor.getColumnIndex("id"))
        //                 value = cursor.getString(cursor.getColumnIndex("value"))
        //                 setTimestamp(cursor.getString(cursor.getColumnIndex("observation_time_stamp")))
        //                 unitDbId = cursor.getString(cursor.getColumnIndex("observation_unit_id"))
        //                 dbId = cursor.getString(cursor.getColumnIndex("observation_db_id"))
        //                 setLastSyncedTime(cursor.getString(cursor.getColumnIndex("last_synced_time")))
        //                 collector = cursor.getString(cursor.getColumnIndex("collector")) ?: ""
        //                 studyId = cursor.getString(cursor.getColumnIndex("study_db_id"))
        //                 variableDbId = cursor.getString(cursor.getColumnIndex("external_db_id"))
        //                 variableName = cursor.getString(cursor.getColumnIndex("observation_variable_name"))
        //                 rep = cursor.getString(cursor.getColumnIndex("rep"))
        //             }
                    
        //             val format = cursor.getString(cursor.getColumnIndex("observation_variable_field_book_format"))
        //             val source = cursor.getString(cursor.getColumnIndex("trait_data_source"))
        ...
        //                     if (isPhoto) "incompleteImageObservations" else "editedObservations" // Non-photos with dbid but no sync time are considered edited
        //                 }
        //                 else -> {
        ...
        //             (categories[category] as MutableList<Observation>).add(observation)
        //         }
        //     }
            
        //     // Return the categorized observations
        //     categories
        // } ?: mapOf(
        //     "newObservations" to emptyList<Observation>(),
        //     "syncedObservations" to emptyList<Observation>(),
        //     "editedObservations" to emptyList<Observation>(),
        //     "newImageObservations" to emptyList<Observation>(),
        //     "syncedImageObservations" to emptyList<Observation>(),
        //     "editedImageObservations" to emptyList<Observation>(),
        //     "incompleteImageObservations" to emptyList<Observation>(),
        //     "userCreatedTraitObservations" to emptyList<Observation>(),
        //     "wrongSourceObservations" to emptyList<Observation>(),
        //     "userCreatedImageObservations" to emptyList<Observation>(),
        //     "wrongSourceImageObservations" to emptyList<Observation>()
        // )
        ...
        */
        ...
        //         if (observation.value.isNullOrEmpty()) return@mapNotNull null
