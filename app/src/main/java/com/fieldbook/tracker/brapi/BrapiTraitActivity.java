...
public class BrapiTraitActivity extends AppCompatActivity {
    ...
    public void saveTraits() {
        ...
        for (int i = 0; i < selectedTraits.size(); ++i) {
            ...
            if (ConfigActivity.dt.hasTrait(trait.trait)) {
                Toast.makeText(getApplicationContext(), "Trait already exists: " + trait.trait, Toast.LENGTH_SHORT).show();
                return;
            }
