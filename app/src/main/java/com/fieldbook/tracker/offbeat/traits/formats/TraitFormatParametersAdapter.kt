...
class TraitFormatParametersAdapter(
    ...
    override fun onCreateViewHolder(
        ...
        val holder =
            format.getTraitFormatDefinition().parameters.find { it.parameter.ordinal == viewType }
                ?.createViewHolder(parent) ?: super.createViewHolder(parent, viewType)
        holders.add(holder)
        return holder
    }

    override fun getItemViewType(position: Int) = currentList[position].parameter.ordinal
